...
/**
 * Get a logger instance. Instance is cached on loggerName+categoryName level.
 * @param  {String} loggerName name of logger to log to.
 * @param  {String} categoryName name of category to log to.
 * @return {Logger} instance of logger for the name with category.
 * @static
 */
function getLoggerNameCategory(loggerName, categoryName) {
    // Use default logger if categoryName is not specified or invalid
  if (typeof categoryName !== "string") {
    categoryName = Logger.DEFAULT_CATEGORY;
  }

  var loggerNameCategoryName = loggerName + categoryName;

  var appenderList;
  if (!loggers[loggerNameCategoryName]) {
    // Create the logger for this name if it doesn't already exist
    loggers[loggerNameCategoryName] = new Logger(loggerName);
    if (appenders[categoryName]) {
      appenderList = appenders[categoryName];
      appenderList.forEach(function(appender) {
        loggers[loggerNameCategoryName].addListener("log", appender);
      });
    }
    if (appenders[ALL_CATEGORIES]) {
      appenderList = appenders[ALL_CATEGORIES];
  ...
  return loggers[loggerNameCategoryName];
}
...
module.exports = {
  getLogger: getLogger,
  getLoggerNameCategory: getLoggerNameCategory,
